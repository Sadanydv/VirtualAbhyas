...
data.forEach((video, i) => {
    let video_element = `
                <div class="video" data-id="${video.id}">
                    <img src="/cp/images/play.svg" alt="">
                    <p>${i + 1 > 9 ? i + 1 : '0' + (i + 1)}. </p>
...
videos[0].querySelector('img').src = '/css/cp/images/pause.svg';
...
videos.forEach(selected_video => {
    selected_video.onclick = () => {
        ...
        for (all_videos of videos) {
            ...
            all_videos.querySelector('img').src = '/css/cp/images/play.svg';

        }

        selected_video.classList.add('active');
        selected_video.querySelector('img').src = '/css/cp/images/pause.svg';

        let match_video = data.find(video => video.id == selected_video.dataset.id);
        main_video.src = '/css/cp/videos/' + match_video.name;
